...
- 어떤 컴포넌트든 자유롭게 접근하는 전역 state를 생성하자.
...
// - 매개변수로 state와 action이 전달된다.
// - action의 모양 {type:string, payload:{id:"", title:"", completed:false}} 위와 같다.
// - action의 모양 {type:string, payload:TodoType} 위와 같다.
